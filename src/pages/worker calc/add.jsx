...
function AddWorkerCalcPage() {
  const location = useLocation();
  const navigate = useNavigate();
  const [loading, setLoading] = useState(false);
  const [data, setData] = useState(false);

  async function AddWorkerCalc() {
    setLoading(true);

    await fetch(SYSTEM_URL + "create_worker_calc/", {
      method: "POST",
      headers: {
      ...
      .then((data) => {
        alert("تم اضافة سجل ");
        navigate("/worker_calcs", { replace: true });
      })
      .catch((error) => {
        console.log(error);
        alert(error + "😕");
  ...
  return (
    <>
      ...
      <hr />
      <div className="container text-center">
        <button className="btn btn-success" onClick={AddWorkerCalc}>
          حفظ الفاتورة
        </button>
      </div>
    </>
...
export default AddWorkerCalcPage;
